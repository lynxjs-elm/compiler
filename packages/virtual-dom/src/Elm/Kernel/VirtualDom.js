...
function _VirtualDom_render(vNode, eventNode)
{
	...
	if (tag === __2_TEXT)
	{
		return _Lynx_createRawText(vNode.__text);
	}
	...
	var domNode = _Lynx_createElement(vNode.__tag);

	_VirtualDom_applyFacts(domNode, eventNode, vNode.__facts);

	for (var kids = vNode.__kids, i = 0; i < kids.length; i++)
	{
		var kid = tag === __2_NODE ? kids[i] : kids[i].b;
		if (kid.$ === __2_TEXT)
		{
			_Lynx_appendChild(domNode, _Lynx_createRawText(kid.__text));
		}
		else
		{
...
function _VirtualDom_applyEvents(domNode, eventNode, events)
{
	...
	for (var key in events)
	{
		...
		var handlerType = __VirtualDom_toHandlerInt(newHandler);
...
function _VirtualDom_applyPatch(domNode, patch)
{
	switch (patch.$)
	{
		...
		case __3_TEXT:
			_Lynx_setAttribute(domNode, 'text', patch.__data);
			return domNode;
		...
		case __3_TAGGER:
			...
			{
				_VirtualDom_getMeta(domNode).elm_event_node_ref.__tagger = patch.__data;
			}
			else
			{
				_VirtualDom_getMeta(domNode).elm_event_node_ref = { __tagger: patch.__data, __parent: patch.__eventNode };
			}
			return domNode;
		...
		case __3_REMOVE_LAST:
			...
			for (var i = 0; i < data.__diff; i++)
			{
				...
				_Lynx_removeElement(domNode, children[data.__length]);
			}
			return domNode;
		...
		case __3_APPEND:
			...
			for (var i = data.__length; i < kids.length; i++)
			{
				_Lynx_appendChild(domNode, _VirtualDom_render(kids[i], patch.__eventNode));
			}
			return domNode;
...
function _VirtualDom_applyPatchReorder(domNode, patch)
{
	...
	// remove end inserts
	var endInsertNodes = _VirtualDom_applyPatchReorderEndInsertsHelp(data.__endInserts, patch);
	...
	for (var i = 0; i < inserts.length; i++)
	{
		...
		var children = _Lynx_getChildren(domNode);
		_Lynx_insertBefore(domNode, node, children[insert.__index]);
	}
...
function _VirtualDom_applyPatchReorderEndInsertsHelp(endInserts, patch)
{
	...
	for (var i = 0; i < endInserts.length; i++)
	{
		...
		var entry = insert.__entry;
		nodes.push(entry.__tag === __5_MOVE
			? entry.__data
			: _VirtualDom_render(entry.__vnode, patch.__eventNode)
		);
	}
