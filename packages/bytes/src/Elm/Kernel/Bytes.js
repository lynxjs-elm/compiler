...
// Add enumerable numeric index properties to a DataView so that
// Elm's structural equality (_Utils_eqHelp) can compare byte contents.
// Without this, DataView has no enumerable keys and any two Bytes
// values are always considered equal by (==).
function _Bytes_wrap(dv)
{
...
function _Bytes_encode(encoder)
{
	...
	return _Bytes_wrap(mutableBytes);
}
...
{
	return __Utils_Tuple2(offset + len, _Bytes_wrap(new DataView(bytes.buffer, offset, len)));
});
